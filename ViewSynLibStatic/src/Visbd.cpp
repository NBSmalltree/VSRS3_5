...
void CViewInterpolation1D::PixelMapping(int x, int y, unsigned char d, float z, unsigned char* RefY, unsigned char* RefU, unsigned char* RefV, int ViewId, double dk, int flooring)
{
  ...
  {
    if (flooring)
      x2 = (int)(floor( (x - dk) * SubPelOption)); //这种情况不存在,故不会执行;
    else
       ...
       y2<Height              && y2>=0    ) // (x2,y2) must fall within the picture
  {
    ...
    nearToCamera = false;
    if (Mask[ViewId][ind2]==NOTHOLEPIXEL) //如果该像素掩模为非空洞区域;
    {
    ...
    {
      //constrain
	    left_pixel = CLIP(x2-1, 0, line_size-1)+y2*line_size; //由于图像本质上是一维排列的, 最后要加上行距;
      ...
      //filter wrong background pixels caused by per-pixel processing 过滤每个像素处理造成的错误背景像素;
      //do not warp the pixel, because it is a suspicious pixel in the gap of foreground object
	    if( Mask[ViewId][left_pixel] == NOTHOLEPIXEL && Mask[ViewId][right_pixel] == NOTHOLEPIXEL && WarpEnhancementOption == 1) //如果Mask左边1像素和右边1像素均为非空洞;
      {
	    ...
	    else if( Mask[ViewId][left_pixel] == NOTHOLEPIXEL && Mask[ViewId][right2_pixel] == NOTHOLEPIXEL && WarpEnhancementOption == 1) //如果Mask左边1像素和右边第2像素均为非空洞;
      {
		...
		else if (Mask[ViewId][left2_pixel] == NOTHOLEPIXEL && Mask[ViewId][right_pixel] == NOTHOLEPIXEL && WarpEnhancementOption == 1) //如果Mask左边第2像素和右边第1像素均为非空洞;
      {
      ...
      //enable to warp current pixel if the pixel is not belong to the area around strong depth edge 如果像素不属于强深度边缘周围的区域，则可以映射当前像素;
      //aiming to clean boundary noise causing by slow depth change or depth edge be a little inside of texture edge 旨在消除由于深度变化缓慢或深度边缘引起的边界噪声;
  ...
  if( WarpToRight == 0) // left view-- from left 2 right
    for (y = 0; y < Height; y++)
      for (x = 0; x < Width2;  x++)
      {
        ...
        if (UpsampleRefs == 2)
          dk = dk + dk;
        else if (UpsampleRefs == 4)//我认为这句话用不到,UpsampleRefs只能为1或2;
          dk = dk * 4;
    ...
    for (y = 0; y < Height; y++)
      for (x = Width2-1; x>=0; x--) //要注意此处--的运用;
      {
        // ind = (x,y) is the pixel position in ref view
        ind = x+y*Width2;
        // Calc real depth z from depth image
...
void CViewInterpolation1D::ForwardWarp() // unsigned char* RefLeft, unsigned char* RefRight, unsigned char* RefDepthLeft, unsigned char* RefDepthRight
{
  ...
  //得到最大、最小深度Maxz, Minz;
  //得到最大、最小视差Maxdk, Mindk;
  //得到深度图分割阈值SplattingDepthThreshold;
  ...
  for (x = 0; x < Width2*SubPelOption;  x++)
  {
    if (Mask[MERGVIEW][ind] == HOLEPIXEL) // This is a starting pixel of a hole
    {
      if (pixelValue < 0) // This is the first pixel of a hole
      {
        ...
        xnhole = x + 1;
        ind2 = ind + 1;
        while (!found && xnhole < Width2*SubPelOption) // 统计一行中有多少个空洞;
        {
...
void CViewInterpolation1D::DetectBoundary( unsigned char* edge, unsigned char* depth, int width, int height, int threshold)
{
  ...
  int edge_growth = BoundaryGrowth;
	...
	{
		k = i*width;
		*(edge+k) = NONE; //0th of each row 每行第一个置为0;
		for(j=1; j<width; j++) //x-(x-1) 和前一个做差;
		{
			ind = k+j;
			diff = *(depth+ind) - *(depth+ind-1);//当前深度像素与前一个像素相减;
			if( diff > threshold ) 
				*(edge+ind) = RISE;
    ...
    for( j=1; j<width-1; j++)
          {
			ind = k +j;
			if( edge[ind]==RISE && edge[ind+1]==NONE && depth[ind+1]-depth[ind]>5 )       mask[ind+1] = 50; // later be changed to RISE //和后一个做差,并且比较两者深度值大小,判断是不是上升边界;
			else if( edge[ind]==FALL && edge[ind-1]==NONE && depth[ind-1]-depth[ind]>5 )  mask[ind-1] = 60; // later be changed to FALL //和前一个做差,并且比较两者深度值大小,判断是不是下降边界;
			else if( edge[ind]==RISE && edge[ind+1]==RISE )                               mask[ind] = 40;   //持续上升边界;
			else if( edge[ind]==FALL && edge[ind-1]==FALL )                               mask[ind] = 40;   // later be changed to NONE //持续下降边界;
          }
		...
		for(j=0;j<width;j++)
		{
			...
			else if( mask[ind] == 40)	//这里认为持续上升或下降的情况不是边界,比如说斜的墙;
			{  edge[ind] = NONE;            	       }
        }

    //to mark 2-pixel-wide area at left side of RISE edge and at right side of FALL edge
    for(j=2;j<width-2;j++)//在RISE边缘的左侧和FALL边缘的右侧标记2像素宽的区域;
    {
      ind = k +j;
      if(edge[ind+1] == RISE || edge[ind+2] == RISE)
        edge[ind] = SUSPECT;//这一步执行完成以后,我认为边界像素是RISE的只有一个像素点了;
      else if(edge[ind-1] == FALL || edge[ind-2] == FALL)
        edge[ind] = SUSPECT;//同上分析，FALL也只有一个像素点了;
      }

    //to mark edge_growth-pixel-wide area around strong depth edge (both sides)
    for(j=edge_growth;j<width-edge_growth;j++)//在强深度边缘（两侧）上标记edge_growth像素范围的区域;
    {
      ind = k +j;
...
int CViewInterpolation1D::DoOneFrame(unsigned char* RefLeft, unsigned char* RefRight, unsigned char* RefDepthLeft, unsigned char* RefDepthRight, unsigned char* Syn)
{
  ...
  Width2 = Width*UpsampleRefs;//实际宽度;
  ...
  if(TemporalImprovementOption == 1)//时间选项;
  {
...
void CViewInterpolation1D::FindDepthMaxMin( unsigned char* map, int ViewID)
{
	...
	for(i=0; i<Height; i=i+16)//此处高度和接下来的宽度以16累加我认为是降低16*16=256倍采样频率，提高算法效率，对16*16块的第一个像素检索足矣深度的最值;
	{
		k = i * Width;
